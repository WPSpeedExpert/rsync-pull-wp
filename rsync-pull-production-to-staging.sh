...
import_methods=("clpctl" "unzip_clpctl" "pv_gunzip" "default")
...
backup_file="${staging_scriptPath}/tmp/${databaseName}-backup-$(date +%F).sql.gz"
if ls ${staging_scriptPath}/tmp/${databaseName}-backup-*.sql.gz 1> /dev/null 2>&1; then
    echo "[+] NOTICE: Deleting older backup files in ${staging_scriptPath}/tmp/" 2>&1 | tee -a ${LogFile}
    rm ${staging_scriptPath}/tmp/${databaseName}-backup-*.sql.gz
    if [ $? -ne 0 ]; then
